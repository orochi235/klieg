...
- **Docstrings that contradict their code:**
  ...
  - Two docstrings in `render/word.ts` sit on the wrong declarations (`WordExtent`, and
    `layoutOpts` instead of `drawsInk`).

### Traps

- **This checkout holds another session's uncommitted work:** the modified
  `specs/2026-09-12-shared-surface-design.md`, the untracked `plans/2026-09-12-shared-surface.md`,
  and `spikes/_mast-tmp.mjs`. None of it is yours. Stage explicit paths only, never `git add -A`.
