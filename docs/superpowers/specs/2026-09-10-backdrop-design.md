...
(`decorations/tube.ts`, at the `takeBlueprint` call), so the same character on two rows keys
differently and each row builds from scratch. This is the row cost, and the seed alone accounts for
all of it.
