...
import { glow, type PowerControl, power, strike, thinning } from '../../src/effects/power.js';
...
describe('power', () => {
  ...
  it('warms up from the frame after up, then comes back on', () => {
    const c = power({ warmup: strike({ duration: 1000 }) });
    c.short();
    draw(c, 0);
    c.up();
  ...
  it('warms up by itself after a timed short, counted from the end of the dark', () => {
    const c = power({ warmup: strike({ duration: 400 }) });
    c.short({ for: 1000 });
    draw(c, 0);
  ...
  it('skips the warm-up under reduced motion', () => {
    const c = power();
    c.short();
    draw(c, 0, Number.POSITIVE_INFINITY);
    c.up();
    expect(draw(c, 16, Number.POSITIVE_INFINITY)).toEqual({});
  });
});
...
describe('power trip', () => {
  ...
  it('trips when a single part holds high while the rest read low', () => {
    const c = power({ trip: { on: (_t, part) => (part.index === 0 ? 1 : 0), holdMs: 1000 } });
  ...
  it('starts the hold over when the reading dips', () => {
    const hover = level(1);
    const c = power({ trip: { on: hover, holdMs: 3000 } });
